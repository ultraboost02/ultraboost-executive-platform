...
export async function POST(request: Request) {
  ...
  const reason = String(body.reason ?? "").trim() || "Votre demande ne répond pas aux critères actuels.";
  ...
  await xanoServerFetch(interpolatePath(admissionUpdatePath, { id: String(admission_id) }), {
    ...
    body: JSON.stringify({ status: "refusee", refused_at: new Date().toISOString(), refusal_reason: reason }),
  });

  if (isEmailConfigured()) {
    const subject = "Votre demande UltraBoost";
    const text =
      `Bonjour ${candidate_first_name || ""},\n\n` +
      `Nous vous remercions pour votre candidature.\n` +
      `Après examen, nous ne pouvons pas la valider pour le moment.\n\n` +
      `Motif : ${reason}\n`;
    try {
